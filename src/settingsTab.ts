...
export default class DailyNotesSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      .addButton(button => {
        button.setButtonText('Add New Daily Note Config')
          .setCta()
          .onClick(() => {
            // Add new config
   ...
   */
  addDailyNoteConfigUI(containerEl: HTMLElement, config: DailyNoteConfiguration, index: number) {
    const setting = new Setting(containerEl);

    // Folder setting
    setting.setName(`Daily Note Config ${index + 1}`)
